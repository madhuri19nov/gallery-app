...
class Gallery extends Component {
  state = {
    activeTumbnailitemId: imagesList[0].id,
  }

  setActiveThumbnailId = id => {
    this.setState({
      activeTumbnailitemId: id,
    })
  }

  render() {
    const {activeTumbnailitemId} = this.state
    const {imageUrl, imageAltText} = imagesList[activeTumbnailitemId]
          ...
          <ul className="thumbnails-list">
            {imagesList.map(eachImage => (
              <ThumbnailItem
                ...
                isActive={activeTumbnailitemId === eachImage.id}
